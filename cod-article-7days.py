...
def get_markdown():
    try:
        with open('summary_output.md', 'r', encoding='utf-8') as f:
            content = f.read()

        return jsonify({'markdown': content}), 200, {'Content-Type': 'text/markdown; charset=utf-8'}
    except Exception as e:
        print('Error reading markdown file:', e)
        return jsonify({'markdown': None}), 500
...
def generate_summary_markdown(articles, search_query):
    markdown_str = f"# Search Query: {search_query}\n\n"
...
if __name__ == '__main__':
    ...
    # generate_summary_markdown(article_urls, search_query)
    app.run(port=5000)